...
def dict2str(result_dict):
    """
    Converts a dictionary (potentially nested) to a formatted string.
    Handles both flat and nested dictionaries for evaluation results.
    """
    ...
    # Check if this is our new nested dictionary (e.g., from grouped evaluation)
    is_nested = all(isinstance(v, dict) for v in result_dict.values())

    if is_nested:
        # Handle the nested dictionary from grouped evaluation
        for group_name, group_metrics in result_dict.items():
            # Add a newline for each group for better readability, except for the first one
            if result_str:
                result_str += '\n'
            
            result_str += f'[{group_name}]'.ljust(10) # Adjust padding for alignment
            
            for metric, value in group_metrics.items():
                result_str += f'{str(metric)}: {value:.4f}    '
    else:
        # Handle the original flat dictionary
        for metric, value in result_dict.items():
            result_str += f'{str(metric)}: {value:.4f}    '

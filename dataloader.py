import math
import torch
import random
import numpy as np
from logging import getLogger
from scipy.sparse import coo_matrix

class AbstractDataLoader(object):
    """
    (This docstring is from your original file)
    """
    def __init__(self, config, dataset, additional_dataset=None,
                 batch_size=1, neg_sampling=False, shuffle=False):
        self.config = config
        self.logger = getLogger()
        self.dataset = dataset
        self.dataset_bk = self.dataset.copy(self.dataset.df)
        self.additional_dataset = additional_dataset
        self.batch_size = batch_size
        self.step = batch_size
        self.shuffle = shuffle
        self.neg_sampling = neg_sampling
        self.device = config['device']

        self.sparsity = 1 - self.dataset.inter_num / self.dataset.user_num / self.dataset.item_num
        self.pr = 0
        self.inter_pr = 0

    def pretrain_setup(self):
        pass

    def data_preprocess(self):
        pass

    def __len__(self):
        return math.ceil(self.pr_end / self.step)

    def __iter__(self):
        if self.shuffle:
            self._shuffle()
        return self

    def __next__(self):
        if self.pr >= self.pr_end:
            self.pr = 0
            self.inter_pr = 0
            raise StopIteration()
        return self._next_batch_data()

    @property
    def pr_end(self):
        raise NotImplementedError('Method [pr_end] should be implemented')

    def _shuffle(self):
        raise NotImplementedError('Method [shuffle] should be implemented.')

    def _next_batch_data(self):
        raise NotImplementedError('Method [next_batch_data] should be implemented.')


class TrainDataLoader(AbstractDataLoader):
    """
    General dataloader with negative sampling.
    """
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        super().__init__(config, dataset, additional_dataset=None,
                         batch_size=batch_size, neg_sampling=True, shuffle=shuffle)

        # Use the authoritative field names from the dataset object
        self.uid_field = self.dataset.uid_field
        self.iid_field = self.dataset.iid_field

        self.history_items_per_u = dict()
        self.all_items = self.dataset.df[self.iid_field].unique().tolist()
        self.all_uids = self.dataset.df[self.uid_field].unique()
        self.all_items_set = set(self.all_items)
        self.all_users_set = set(self.all_uids)
        
        self.use_full_sampling = getattr(config, 'use_full_sampling', False)
        use_neg_sampling = getattr(config, 'use_neg_sampling', True)

        # =======================================================================
        # >>>>>>>>>>>>>>>>>>>>>>>> 核心修改点 1 <<<<<<<<<<<<<<<<<<<<<<<
        # =======================================================================
        # Safely check if hard negative sampling is enabled
        self.use_hns = getattr(config, 'use_hard_negative_sampling', False)
        # =======================================================================
        
        if use_neg_sampling:
            if self.use_full_sampling:
                self.sample_func = self._get_full_uids_sample
            else:
                self.sample_func = self._get_neg_sample
        else:
            self.sample_func = self._get_non_neg_sample

        self._get_history_items_u()
        
        self.neighborhood_loss_required = getattr(config, 'use_neighborhood_loss', False)
        if self.neighborhood_loss_required:
            self.history_users_per_i = {}
            self._get_history_users_i()
            self.user_user_dict = self._get_my_neighbors(getattr(config, 'USER_ID_FIELD', 'user_id'))
            self.item_item_dict = self._get_my_neighbors(getattr(config, 'ITEM_ID_FIELD', 'item_id'))

    def pretrain_setup(self):
        if self.shuffle:
            self.dataset = self.dataset_bk.copy(self.dataset_bk.df)
        self.all_items.sort()
        if self.use_full_sampling:
            self.all_uids.sort()
        random.shuffle(self.all_items)

    def inter_matrix(self, form='coo', value_field=None):
        if not self.dataset.uid_field or not self.dataset.iid_field:
            raise ValueError('dataset doesn\'t exist uid/iid, thus can not converted to sparse matrix')
        return self._create_sparse_matrix(self.dataset.df, self.dataset.uid_field,
                                          self.dataset.iid_field, form, value_field)

    def _create_sparse_matrix(self, df_feat, source_field, target_field, form='coo', value_field=None):
        src = df_feat[source_field].values
        tgt = df_feat[target_field].values
        data = np.ones(len(df_feat)) if value_field is None else df_feat[value_field].values
        mat = coo_matrix((data, (src, tgt)), shape=(self.dataset.user_num, self.dataset.item_num))
        return mat if form == 'coo' else mat.tocsr()

    @property
    def pr_end(self):
        if self.use_full_sampling:
            return len(self.all_uids)
        return len(self.dataset)

    def _shuffle(self):
        self.dataset.shuffle()
        if self.use_full_sampling:
            np.random.shuffle(self.all_uids)

    def _next_batch_data(self):
        return self.sample_func()

    def _get_neg_sample(self):
        cur_data = self.dataset[self.pr: self.pr + self.step]
        self.pr += self.step
        
        user_tensor = torch.tensor(cur_data[self.uid_field].values, dtype=torch.long)
        item_tensor = torch.tensor(cur_data[self.iid_field].values, dtype=torch.long)

        # =======================================================================
        # >>>>>>>>>>>>>>>>>>>>>>>> 核心修改点 2 <<<<<<<<<<<<<<<<<<<<<<<
        # =======================================================================
        # If Hard Negative Sampling is enabled, return a tuple with None for neg_item.
        # This signals the Trainer to generate hard negatives dynamically.
        if self.use_hns:
            return (user_tensor, item_tensor, None)

        # Otherwise, perform the original random negative sampling logic.
        user_tensor = user_tensor.to(self.device)
        item_tensor = item_tensor.to(self.device)
        batch_tensor = torch.cat((torch.unsqueeze(user_tensor, 0), torch.unsqueeze(item_tensor, 0)))
        
        u_ids = cur_data[self.uid_field]
        neg_ids = self._sample_neg_ids(u_ids).to(self.device)
        
        if self.neighborhood_loss_required:
            i_ids = cur_data[self.iid_field]
            pos_neighbors, neg_neighbors = self._get_neighborhood_samples(i_ids, self.iid_field)
            pos_neighbors, neg_neighbors = pos_neighbors.to(self.device), neg_neighbors.to(self.device)
            batch_tensor = torch.cat((batch_tensor, neg_ids.unsqueeze(0),
                                      pos_neighbors.unsqueeze(0), neg_neighbors.unsqueeze(0)))
        else:
            batch_tensor = torch.cat((batch_tensor, neg_ids.unsqueeze(0)))

        return batch_tensor
        # =======================================================================

    def _get_non_neg_sample(self):
        cur_data = self.dataset[self.pr: self.pr + self.step]
        self.pr += self.step
        user_tensor = torch.tensor(cur_data[self.uid_field].values, dtype=torch.long)
        item_tensor = torch.tensor(cur_data[self.iid_field].values, dtype=torch.long)
        batch_tensor = torch.cat((torch.unsqueeze(user_tensor, 0), torch.unsqueeze(item_tensor, 0)))
        return batch_tensor

    def _get_full_uids_sample(self):
        user_tensor = torch.tensor(self.all_uids[self.pr: self.pr + self.step], dtype=torch.long).to(self.device)
        self.pr += self.step
        return user_tensor

    def _sample_neg_ids(self, u_ids):
        neg_ids = []
        for u in u_ids:
            iid = random.choice(self.all_items)
            while iid in self.history_items_per_u.get(u, set()):
                iid = random.choice(self.all_items)
            neg_ids.append(iid)
        return torch.tensor(neg_ids, dtype=torch.long)

    def _get_my_neighbors(self, id_str):
        # This function seems to use config, let's make it robust
        id_field_clean = id_str.split(':')[0]
        a2b_dict = self.history_items_per_u if id_field_clean == self.uid_field else self.history_users_per_i
        b2a_dict = self.history_users_per_i if id_field_clean == self.uid_field else self.history_items_per_u
        ret_dict = {}
        for i, j in a2b_dict.items():
            k = set()
            for m in j:
                k |= b2a_dict.get(m, set()).copy()
            k.discard(i)
            ret_dict[i] = k
        return ret_dict

    def _get_neighborhood_samples(self, ids, id_str):
        id_field_clean = id_str.split(':')[0]
        a2a_dict = self.user_user_dict if id_field_clean == self.uid_field else self.item_item_dict
        all_set = self.all_users_set if id_field_clean == self.uid_field else self.all_items_set
        pos_ids, neg_ids = [], []
        for i in ids:
            pos_ids_my = a2a_dict.get(i, set())
            if not pos_ids_my or len(pos_ids_my) / len(all_set) > 0.8:
                pos_ids.append(0)
                neg_ids.append(0)
                continue
            pos_id = random.sample(pos_ids_my, 1)[0]
            pos_ids.append(pos_id)
            neg_id = random.sample(all_set, 1)[0]
            while neg_id in pos_ids_my:
                neg_id = random.sample(all_set, 1)[0]
            neg_ids.append(neg_id)
        return torch.tensor(pos_ids, dtype=torch.long), torch.tensor(neg_ids, dtype=torch.long)

    def _random(self):
        return random.choice(self.all_items)

    def _get_history_items_u(self):
        uid_freq = self.dataset.df.groupby(self.uid_field)[self.iid_field]
        for u, u_ls in uid_freq:
            self.history_items_per_u[u] = set(u_ls.values)
        return self.history_items_per_u

    def _get_history_users_i(self):
        uid_freq = self.dataset.df.groupby(self.iid_field)[self.uid_field]
        for i, u_ls in uid_freq:
            self.history_users_per_i[i] = set(u_ls.values)
        return self.history_users_per_i

class EvalDataLoader(AbstractDataLoader):
    def __init__(self, config, dataset, additional_dataset=None, batch_size=1, shuffle=False):
        super().__init__(config, dataset, additional_dataset, batch_size, False, shuffle)

        if additional_dataset is None:
            raise ValueError('Training dataset is required for evaluation.')
        
        self.uid_field = self.dataset.uid_field
        self.iid_field = self.dataset.iid_field
        
        
        self.eval_items_per_u = []
        self.eval_len_list = []
        self.train_pos_len_list = []

        self.eval_u = self.dataset.df[self.uid_field].unique()
        self.pos_items_per_u = self._get_pos_items_per_u(self.eval_u)
        self._get_eval_items_per_u(self.eval_u)
        self.eval_u = torch.tensor(self.eval_u, dtype=torch.long).to(self.device)

    @property
    def pr_end(self):
        return self.eval_u.shape[0]

    def _shuffle(self):
        self.dataset.shuffle()

    def _next_batch_data(self):
        inter_cnt = sum(self.train_pos_len_list[self.pr: self.pr + self.step])
        batch_users = self.eval_u[self.pr: self.pr + self.step]
        batch_mask_matrix = self.pos_items_per_u[:, self.inter_pr: self.inter_pr + inter_cnt].clone()
        batch_mask_matrix[0] -= self.pr
        self.inter_pr += inter_cnt
        self.pr += self.step
        return [batch_users, batch_mask_matrix]

    def _get_pos_items_per_u(self, eval_users):
        uid_field = self.additional_dataset.uid_field
        iid_field = self.additional_dataset.iid_field
        uid_freq = self.additional_dataset.df.groupby(uid_field)[iid_field]
        u_ids = []
        i_ids = []
        for i, u in enumerate(eval_users):
            if u in uid_freq.groups:
                u_ls = uid_freq.get_group(u).values
                i_len = len(u_ls)
                self.train_pos_len_list.append(i_len)
                u_ids.extend([i] * i_len)
                i_ids.extend(u_ls)
            else:
                 self.train_pos_len_list.append(0)
        return torch.tensor([u_ids, i_ids], dtype=torch.long)

    def _get_eval_items_per_u(self, eval_users):
        uid_freq = self.dataset.df.groupby(self.uid_field)[self.iid_field]
        for u in eval_users:
            if u in uid_freq.groups:
                u_ls = uid_freq.get_group(u).values
                self.eval_len_list.append(len(u_ls))
                self.eval_items_per_u.append(u_ls)
            else:
                self.eval_len_list.append(0)
                self.eval_items_per_u.append(np.array([]))
        self.eval_len_list = np.asarray(self.eval_len_list)

    def get_eval_items(self):
        return self.eval_items_per_u
    def get_eval_len_list(self):
        return self.eval_len_list
    def get_eval_users(self):
        return self.eval_u.cpu()